...
/**
 * A representation of the most simple networking device available.
 * This device simply replays any communication from any one port to all 
 * other ports. This device is not often used in any kind of large
 * scale network because of the flooding effect it has on that network.
 * Once a communication is send by one computer on the network, no other 
...
public class Hub extends Infrastructure implements Serializable
{
	...
	/* 
	 * NOTES - Connections between this device and the devices connected to it will be
	 * handled by the main system.
	 */ 
