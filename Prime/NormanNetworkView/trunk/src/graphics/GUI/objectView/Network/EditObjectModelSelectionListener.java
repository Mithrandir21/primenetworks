 ...
 * 
 */
public class EditObjectModelSelectionListener implements TableModelListener
{

	private JTable table;

	private HashMap<String, Permissions> permissions;

	// It is necessary to keep the table since it is not possible
	// to determine the table from the event's source
	public EditObjectModelSelectionListener(JTable table,
			HashMap<String, Permissions> permissions)
	{
		this.table = table;
	 ...
	 * 
	 */
	public void addObjectPermissions(String groupName, boolean right, int column)
	{
		if ( groupName != null && column > 0 && column < 4 )
		{
			// Gets the permission from the map
			Permissions perm = permissions.get(groupName);
			...
			else
			{
				...
				// Puts the new permissions
				permissions.put(groupName, newPerm);
			}
		}
	}
}
